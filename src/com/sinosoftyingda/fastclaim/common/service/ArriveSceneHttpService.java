...
import com.sinosoftyingda.fastclaim.common.model.ArriveSceneRequest;
 ...
 * 
 */
public class ArriveSceneHttpService {
	/**
	 * 到达现场接口方法
	 * 
	 * @param responseDuty
	 * @return
	 */
	public static CommonResponse arriveSceneService(
			ArriveSceneRequest arriveSceneRequest, String url) {
		CommonResponse responseDuty = new CommonResponse();
		try {
			String requestXML = createArriveSceneRequest(arriveSceneRequest);
			Log.i("mCliam", "到达现场------------->requestXML:" + requestXML);
			...
			// "</HEAD>" + "<BODY>"
			// "</BODY>" + "</PACKET>";
			Log.i("mCliam", "到达现场------------->responseXML:" + responseXML);

			responseDuty = arriveSceneResponseXml(responseXML);
			/**
			 * 保存请求报文
			 */
...
////			TblInfoBuffer.addinfobuffer(infoBuffer);
	 ...
	 * @throws Exception
	 */
	private static String createArriveSceneRequest(
			ArriveSceneRequest arriveSceneRequest) throws Exception {
		...
		serializer.text(arriveSceneRequest.getUserCode());
		serializer.endTag("", "USERCODE");
		// 节点类型
		serializer.startTag("", "NODETYPE");
		serializer.text(arriveSceneRequest.getPassWord());
		serializer.endTag("", "NODETYPE");
		// 损失项目编号
		serializer.startTag("", "LOSSNO");
		serializer.text(arriveSceneRequest.getFlag());
		serializer.endTag("", "LOSSNO");
		// 报案号
		serializer.startTag("", "REGISTNO");
		serializer.text(arriveSceneRequest.getRegistNo());
	 ...
	 * 
	 */
	private static CommonResponse arriveSceneResponseXml(String responseXml)
			throws Exception {
		...
		CommonResponse responseArriveScene = new CommonResponse();
		/**
		 * 响应头
		 */
		if (responseXml == null) {
			responseArriveScene.setResponseMessage("服务器返回数据错误!请联系管理员!(终端)");
		} else {
			if (responseXml.equals("Timeout")) {
				responseArriveScene.setResponseMessage("移动端与快赔系统连接超时,请检查网络后重新连接!(终端)");

			} else if (responseXml.equals("Post") || responseXml.equals("")) {
				responseArriveScene.setResponseMessage("移动端与快赔连接失败,请联系管理员!(终端)");
			} else {
				/**
					...
					if (eventType == XmlPullParser.START_DOCUMENT) {// 开头
						...
						if (!startTag.equals("PACKET")
							...
							if (startTag.equalsIgnoreCase("RESPONSETYPE")) {
								responseArriveScene.setReaponseType(value);
							} else if (startTag
									.equalsIgnoreCase("RESPONSECODE")) {
								responseArriveScene.setResponseCode(value);
							} else if (startTag
									.equalsIgnoreCase("RESPONSEMESSAGE")) {
								responseArriveScene.setResponseMessage(value);
							}
							eventType = parser.next();
						} else {
							eventType = parser.next();
