 ...
 * 
 */
public class PrintORMsgHttpService {
	/**
	 * 打印接口方法
	 * 
	 * @param responseDuty
	 * @return
	 */
	public static PrintResponse printORMsgService(PrintRequest printRequest,String url) {
		PrintResponse responsePrint = new PrintResponse();
		try {
			String requestXML = createPrintRequest(printRequest);
			Log.i("mCliam", "打印------------->requestXML:" + requestXML);
			...
			// "</HEAD>" + "<BODY>"
			// "</BODY>" + "</PACKET>";
			Log.i("mCliam", "打印------------->responseXML:" + responseXML);
...
//			TblInfoBuffer.addinfobuffer(infoBuffer);
	...
	private static String createPrintRequest(PrintRequest printRequest)
		...
		serializer.text("VerifyLossPrintAndMsg");
		...
		serializer.text(printRequest.getUserCode());
		serializer.endTag("", "USERCODE");
		// 是否发送短信
		serializer.startTag("", "ISSENDMESSAGE");
		serializer.text(printRequest.getIsSendMessage());
		serializer.endTag("", "ISSENDMESSAGE");
		// 是否打印
		serializer.startTag("", "ISPRINT");
		serializer.text(printRequest.getIsPrint());
		serializer.endTag("", "ISPRINT");
		// 报案号
	...
	private static PrintResponse printResponseXml(String responseXml)
		...
		if (responseXml == null) {
			...
			if (responseXml.equals("Timeout")) {
				printResponse.setResponseMessage("连接超时!(终端)");

			} else if (responseXml.equals("Post") || responseXml.equals("")) {
				printResponse.setResponseMessage("连接失败!(终端)");
			} else {
				/**
